"""
SignBridge — Phase 6: Hybrid Inference System
...
- TFLite dynamic model integration
...
# TFLite for dynamic model inference
try:
    ...
    try:
        ...
        print("❌ TFLite not available. Please install tflite-runtime or tensorflow")
...
DYNAMIC_MODEL_PATH = "models/signbridge_dynamic.tflite"
...
class HybridInferenceApp:
    def __init__(self):
        ...
        print("🚀 Starting SignBridge Hybrid Inference System...")
    ...
    def initialize_components(self):
        ...
        try:
            ...
            # Load dynamic model and labels
            if TFLITE_AVAILABLE and os.path.exists(DYNAMIC_MODEL_PATH):
                try:
                    # Try to load with Flex delegate for complex operations
                    self.dynamic_interpreter = Interpreter(
                        model_path=DYNAMIC_MODEL_PATH,
                        experimental_delegates=[tf.lite.experimental.load_delegate('libtensorflowlite_flex.so')] if hasattr(tf.lite.experimental, 'load_delegate') else None
                    )
                except:
                    # Fallback to standard interpreter
                    self.dynamic_interpreter = Interpreter(model_path=DYNAMIC_MODEL_PATH)
                
                self.dynamic_interpreter.allocate_tensors()
                
                # Get input/output details
                self.dynamic_input_details = self.dynamic_interpreter.get_input_details()
                self.dynamic_output_details = self.dynamic_interpreter.get_output_details()
                ...
                print(f"✅ Dynamic model loaded: {len(self.dynamic_labels)} classes")
            else:
    ...
    def predict_dynamic_phrase(self, frame_sequence):
        """Predict dynamic phrase using TFLite model."""
        try:
            ...
            # Set input tensor
            self.dynamic_interpreter.set_tensor(self.dynamic_input_details[0]['index'], input_data)
            
            # Run inference
            self.dynamic_interpreter.invoke()
            
            # Get output tensor
            output_data = self.dynamic_interpreter.get_tensor(self.dynamic_output_details[0]['index'])
    ...
    def draw_hybrid_ui(self, frame):
        ...
        # Draw instructions
        instructions = [
            "Move hand to record phrase",
            "Stop moving to predict",
            "ESC or Q to quit"
        ]
        
        for i, instruction in enumerate(instructions):
            cv2.putText(frame, instruction, (width - 300, 30 + i * 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    ...
    def run(self):
        ...
        print("✨ Hybrid system ready for static + dynamic recognition!")
