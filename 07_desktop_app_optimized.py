"""
SignBridge — Phase 4: Professional Desktop GUI (Optimized)
Optimized version with FPS improvements and 'p' key functionality

Performance optimizations:
- Frame skipping for prediction (every 3rd frame)
- Reduced UI drawing overhead
...
UI_UPDATE_SKIP_FRAMES = 1   # Update UI every frame (no skipping to prevent flicker)

# Disable GPU to prevent conflicts
os.environ['CUDA_VISIBLE_DEVICES'] = '-1'

class SignBridgeOptimizedApp:
    def __init__(self):
        """Initialize the optimized desktop application."""
        print("🚀 Starting SignBridge Optimized Desktop Application...")
    ...
    def extract_hand_landmarks(self, frame):
        """Extract hand landmarks from frame (optimized)."""
        try:
    ...
    def predict_letter(self, landmarks):
        """Predict ASL letter using Keras model (optimized)."""
        try:
            # Prepare input
    ...
    def draw_hand_landmarks(self, frame, hand_landmarks):
        """Draw hand landmarks on frame (optimized)."""
    ...
    def draw_optimized_ui(self, frame):
        """Draw optimized UI overlay on frame (faster version)."""
        height, width = frame.shape[:2]
        ...
        # Draw panel background (single rectangle for better performance)
    ...
    def run(self):
        """Main application loop (optimized)."""
        print("🎉 SignBridge Optimized Desktop Application is ready!")
        print("=" * 60)
        ...
        print("⚡ Optimized for better FPS performance!")
        ...
        while True:
            try:
                ...
                # Optimization: Only process landmarks every frame (needed for drawing)
                landmarks, hand_landmarks = self.extract_hand_landmarks(frame)
                
                # Optimization: Only predict every Nth frame
                prediction = None
                confidence = 0.0
                ...
                # Draw professional UI overlay (every frame to prevent flicker)
                frame = self.draw_optimized_ui(frame)
                
                # Display frame
                cv2.imshow("SignBridge Optimized - ASL Recognition System", frame)
        ...
        # Cleanup
        print("\n👋 Shutting down SignBridge Optimized...")
        self.cap.release()
        if self.landmarker:
